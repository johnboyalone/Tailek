...
const SetupScreen: React.FC<SetupScreenProps> = ({ settings, playerName, onSetupComplete, allPlayersReady, isReady }) => {
      ...
      <div className="w-full max-w-sm text-center">
        {isReady ? (
            <div className="bg-slate-800 p-8 rounded-xl shadow-lg text-center">
                <h2 className="text-3xl font-bold text-green-400 mb-4">ตั้งค่าเลขลับเรียบร้อย!</h2>
                <p className="text-slate-300 text-lg animate-pulse">
                    {allPlayersReady ? 'กำลังเริ่มเกม...' : 'รอผู้เล่นคนอื่นตั้งค่า...'}
                </p>
            </div>
        ) : (
            <>
